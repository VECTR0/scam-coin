...
peerServer.newTransactionCallback = (serializedTransaction) => {
  ...
  if (!transPool.hasTrans(transaction)) {
    //TODO: check if transaction already in blockchain
    try {
...
peerServer.getTransactionsPoolCallback = () => {
  return transPool
    .getAll()
    .map((transaction) => Transaction.serialize(transaction));
};

rl.on('line', (input) => {
  const [command, ...args] = input.split(' ');
  switch (
    command //TODO: refactor to fancy dynamic options list
  ) {
    case 'debug':
    ...
    case 'trans': {
      ...
      if (amount <= 0) {
        // TODO: check in processing new block
    ...
    case 'mine': {
      ...
      if (transaction === null) {
        ...
        transaction = blockchain.createCoinbaseTransaction(
          wallet.getIdentities()[0].address,
        );
      } else {
        transaction.addReward(
          wallet.getIdentities()[0].address,
